...
# ---- Java registration fragments (Stage 2: true in-file Cog) ----
# Two orderings exist in the hand-written code:
#   items use interleaved (wood-major) order; entities group by kind (all boats, then chest boats).
...
def variants_by_kind():
    """kind-major order: boat(crimson,warped) then chest_boat(crimson,warped)."""
    for kind in KINDS:
        for wood in WOODS:
            yield wood, kind, "{0}_{1}".format(wood, kind)


def _predicate(var, names):
    """Render: return var == A || var == B  /  (cont) || var == C || var == D;  (2 terms per line)."""
    terms = ["{0} == {1}".format(var, n) for n in names]
    pairs = [terms[i:i + 2] for i in range(0, len(terms), 2)]
    lines = ["        return " + " || ".join(pairs[0])]
    for p in pairs[1:]:
        lines.append("                || " + " || ".join(p))
    lines[-1] = lines[-1] + ";"
    return lines
...
def items_fields():
    return ["    public static Item {0};".format(const(n)) for _, _, n in variants()]


def items_init():
    lines = []
    for _, _, n in variants():
        lines.append('        {0} = register("{1}", () -> ModEntities.{0});'.format(const(n), n))
    tab = ", ".join(const(n) for _, _, n in variants())
    lines.append("        TAB_ITEMS = List.of({0});".format(tab))
    return lines


def items_predicate():
    return _predicate("i", [const(n) for _, _, n in variants()])


def entities_fields():
    lines = []
    for _, kind, n in variants_by_kind():
        typ = "Boat" if kind == "boat" else "ChestBoat"
        lines.append("    public static EntityType<{0}> {1};".format(typ, const(n)))
    return lines
...
def entities_init():
    lines = []
    for _, kind, n in variants_by_kind():
        fn = "registerBoat" if kind == "boat" else "registerChestBoat"
        lines.append('        {0} = {1}("{2}", () -> ModItems.{0});'.format(const(n), fn, n))
    return lines


def entities_predicate():
    return _predicate("type", [const(n) for _, _, n in variants_by_kind()])