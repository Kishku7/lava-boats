# build-all-fabric.ps1 -- Lava Boats unified Fabric source for every 26.x target.
...
foreach($v in $Versions){
  ...
  Write-Host "=== LB Fabric $v (mc=$($m.mc)) ==="
  ...
  $jar=Get-ChildItem (Join-Path $fabric "build\libs") -Filter "lava-boats-*.jar"|?{$_.Name -notmatch 'sources'}|Sort-Object LastWriteTime|Select-Object -Last 1
  Copy-Item $jar.FullName (Join-Path $dist ("lava-boats-{0}+{1}-fabric.jar" -f $modver,$v)) -Force
  Write-Host "  -> $v done"
}
